...
func (server *Server) renewAccessToken(ctx *gin.Context) {
	...
	if refreshTokenPayload.TokenType != token.RefreshToken {
	...
	accessToken, accessTokenPayload, err := server.tokenMaker.CreateToken(refreshTokenPayload.Username, token.AccessToken, server.config.AccessTokenDuration)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, errorResponse(err))
		return
	}
